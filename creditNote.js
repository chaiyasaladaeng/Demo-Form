...
module.exports = function createCreditNote(doc, imageLogo, documentPDF, generateOnCloudFunction) {
...
function generateCustomerInformation(doc, invoice) {
  ...
  let lineEndCustomerInfo = 195
  generateHr(doc, lineEndCustomerInfo + 3)
  ...
  doc
      ...
      .text("เหตุผลการลดหนี้ /", globalVariable.startY + 360, lineEndCustomerInfo + 5)
...
function generateInvoiceTable(doc, invoice, hrLinePosition) {
  ...
  const itemsPositionX = {
    ...
    unitPrice: globalVariable.startY + 280,
    quantity: globalVariable.startY + 340,
    unit: globalVariable.startY + 430,
    amount: 0
  }
...
function generateRemark(doc, data) {
  let y = globalVariable.startY + 15
  doc.fontSize(globalVariable.fontSize).text("เหตุผลการลดหนี้ / Reason : " + data.reason, y, currentPosition + 15)
}
